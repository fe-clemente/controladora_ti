...
// ─── Pasta raiz do módulo TI ──────────────────────────────────────────────────
// ⚠️  Defina TI_UPLOADS_PASTA_ID no .env com o ID da pasta do Drive do TI
//     Ex: TI_UPLOADS_PASTA_ID=1ABC...XYZ
const PASTA_RAIZ_ID = process.env.TI_UPLOADS_PASTA_ID || process.env.UPLOADS_PASTA_ID || '';
...
async function listarPastas(pastaId = PASTA_RAIZ_ID) {
  if (!pastaId) throw new Error('TI_UPLOADS_PASTA_ID não configurado no .env');
...
async function uploadArquivo({ nomeArquivo, mimeType, buffer, pastaId }) {
  if (!PASTA_RAIZ_ID) throw new Error('TI_UPLOADS_PASTA_ID não configurado no .env');
  const drive   = getDrive();
